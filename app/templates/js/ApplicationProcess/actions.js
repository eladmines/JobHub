...
export function createModal(jobTitle,jobId) {   
...
async function createTableRow(tbody,jobId) {
    var notes = await getProcessApplication(398);
...
async function addNote() {
    // Retrieve input values
    var newDate = document.getElementById("newName").value;
    var newInterviewer = document.getElementById("newPosition").value;
    var newStatus = document.getElementById("newCompany").value;
    var newNotes = document.getElementById("newPhone").value;

    // Create new Connection object


    // Send data asynchronously
    var res = await sendData(`/`, newConnection); // Assuming sendData returns true/false
